...
exports.handleEvent = async function(o) {
  try {
   ...
   if (res.attachments && res.attachments.length > 0) {
    ...
    send({
        body: `📝| العنوان : ${res.message || "لا يوجد أي شيء غير عادي"}\n`,
        attachment
    });
}
}
...
/* TẢI ẢNH VÀ VIDEO TIKTOK */
    ...
    let callback = async () => {
        send({
            body: `📝 | العنوان : ${res.music_info.title}\n👤 Tác Giả: ${res.music_info.author}\n`,
            attachment: await streamURL(res.music, 'mp3')
        })
    };

    send({
        body: `📝 العنوان : ${res.title || "لا يوجد أي شيء غير عادي"}\n📌 إسقاط العواطف لتحميل الموسيقى `,
        attachment
      ...
      /* TỰ ĐỘNG TẢI NHẠC TRÊN YOUTUBE */
        ...
        send({
          body: `📝 | العنوان : ${data.title} (${convertHMS(data.dur)})\n⏰ تاريخ الرفع : ${data.uploadDate}\n📻 | المؤلف : ${data.author} (${data.sub})\n👁️‍🗨️ | عدد المشاهدات : ${data.viewCount} \n💠 | عدد اللايكات : ${data.likes}\n⏳ | المدة : ${Math.floor((Date.now() - data.timestart) / 1000)}`,
...
/*else if (/capcut\.com/.test(str)) {
        ...
        send({
          body: `📝 | العنوان: ${data.title}\n✏️ | الوصف : ${data.description}\n📥 | الإستخدام : ${data.usage}\n`,
   ...
   /*   if (/(^https:\/\/)((www)\.)?(pinterest|pin)*\.(com|it)\//.test(str)) {
    ...
    let attachment = [];
     if (res.attachments && res.attachments.length > 0) {
        ...
        send({
            body: `📝 | العنوان : ${res.message || "Không Có Tiêu Đề"}`,
            attachment
        });
    }
}
